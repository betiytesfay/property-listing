...
export function Navbar() {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Search */}
            <div
              className={`relative flex items-center transition-all duration-300 ease-in-out ${showSearch ? "w-40 sm:w-56" : "w-11"
                }`}
            >
              <div className="flex items-center w-full relative">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Search..."
                  className={`h-11 flex-1 rounded-full border border-slate-200 bg-slate-50 px-4 text-sm outline-none transition-all focus:border-amber-400 focus:bg-white ${showSearch ? "opacity-100" : "opacity-0 pointer-events-none"
                    }`}
                />
              </div>
            </div>
      ...
      {/* Drawer */}
      <aside
        className={`fixed top-0 right-0 z-50 h-full w-[300px] max-w-[85vw] bg-white shadow-2xl transition-transform duration-300 ease-in-out lg:hidden flex flex-col ${mobileOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        {/* Drawer Header */}
